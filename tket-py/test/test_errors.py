...
def test_unresolved_op() -> None:
    """Define a function with an unresolved op, and try to convert it to a TkProgram."""
    ...
    # Loading the TkProgram errors out since it cannot find the extension.
    with pytest.raises(RuntimeError) as excinfo:
        TkProgram.from_bytes(hugr.to_bytes())

    # The error contains the full traceback, rather than just the top-level error message.
    err = str(excinfo.value)
    assert "Could not read TkProgram from bytes" in err
