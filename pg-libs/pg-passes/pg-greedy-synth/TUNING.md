...
| `TQE_SAMPLE_FRACTION` | `0.1`  | Samples roughly 10% of the candidates for a Pauli string with maximum weight. The minimum pool size still applies. |

## Internal heuristics

| Parameter | Value | Purpose | Source |
| --- | --- | --- | --- |
| `PARALLEL_COSTING_THRESHOLD` | `150_000` | Auto mode uses parallel costing when `work_items * candidates >= 150_000`, to offset parallel overhead. `work_items` is the number of visible operations or tableau generators being costed. | [parallel_mode.rs](src/synthesis/parallel_mode.rs) |
...
| `ALPHA` | `0.588` | Discounts the weight of each successive set, favouring nearby operations. | [greedy.rs](src/backend/greedy.rs) |
