...
- **Nightly toolchain**: The `simd` feature available to some crates requires a nightly Rust toolchain. If using the devenv shell, switch to the nightly profile (`devenv shell nightly`) so the toolchain is picked up automatically; otherwise use `cargo +nightly-2026-09-22 ...` as pinned in the `justfile`.
