...
- **Nightly toolchain**: The `simd` feature requires the nightly toolchain selected by `nightly_toolchain` in the root [`justfile`](../justfile). Run `just test-rust-nightly`; in devenv, use `devenv shell nightly`. Ordinary Cargo commands use stable Rust, as specified in [`rust-toolchain.toml`](../rust-toolchain.toml).
