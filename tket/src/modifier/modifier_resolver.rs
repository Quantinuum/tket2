...
mod tests {
    ...
    pub(crate) fn test_modifier_resolver(
        ...
        let h = resolved_modifier_test_hugr(target_num, ctrl_num, foo, dagger, &name);
        std::fs::write(
            format!("{}_solved.mmd", name.as_ref().display()),
            h.mermaid_string(),
        )
        .unwrap();
    }
    ...
    pub(crate) fn resolved_modifier_test_hugr(
        ...
        h.validate().unwrap();
        std::fs::write(
            format!("{}.mmd", name.as_ref().display()),
            h.mermaid_string(),
        )
        .unwrap();
    ...
    #[rstest::rstest]
    #[case::tmp("../test_files/modifier_examples/tmp.hugr")]
