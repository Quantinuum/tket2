...
impl<T> ComputeType<T> {
    ...
    pub(crate) fn get_type(
        ...
        self.custom_type(extension_id, extension_version, extension_ref).into()
    }
    ...
    pub(crate) fn custom_type(
        ...
        match self {
            ...
            Self::Func { inputs, outputs } => {
                Self::func_custom_type(inputs.clone(), outputs.clone(), extension_id, extension_version, extension_ref)
            }
            Self::Result { outputs } => {
                Self::result_custom_type(outputs.clone(), extension_id, extension_version, extension_ref)
            }
    ...
    ($ext_id:expr, $ext_ver:expr, $ext:ty, $opdef:ident) => {
        ...
        impl MakeOpDef for $opdef {
            ...
            fn init_signature(&self, extension_ref: &Weak<Extension>) -> SignatureFunc {
                let context_type =
                    ComputeType::<$ext>::Context.get_type(self.extension(), $ext_ver, extension_ref);
                let module_type =
        ...
        impl ComputeOp<$ext> {
            ...
            fn get_context_return_type(
                ...
                option_type(vec![
                    ComputeType::<$ext>::Context.get_type(extension_id, $ext_ver, extension_ref),
                ])
            }
        }
