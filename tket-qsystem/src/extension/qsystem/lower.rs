...
use hugr::builder::{
    Container, DFGBuilder, DataflowSubContainer, HugrBuilder, SubContainer, inout_sig,
};
use hugr::core::Visibility;
use hugr::extension::prelude::{Barrier, Noop, bool_t, option_type, usize_t};
...
use hugr::std_extensions::collections::array::{
    self, ARRAY_CLONE_OP_ID, ARRAY_DISCARD_OP_ID, GenericArrayOpDef, array_type,
};
...
/// Register any replacements needed to replace ops that require copyable type bounds
/// with ops that can work with non-copyable types, required as part of replacing
/// `Measurement` with `Future`.
/// TODO: While this is the only replacement pass that currently requires dealing with a
/// change in linearity, this could potentially be moved into a more general location.
fn replace_array_ops_requiring_copyable_bounds(lowerer: &mut ReplaceTypes) {
...
pub fn lower_tk2_ops(
    ...
    let mut lowerer = ReplaceTypes::default().with_scope(scope.clone());
...
mod test {
    use hugr::{
        ...
        extension::simple_op::{HasDef, MakeOpDef},
        ops::OpType,
